...
const LanguageModal = ({ modalVisible, setModalVisible, currentTheme, showCrossButton, dontClose }) => {
  ...
  async function determineInitialLanguage() {
    try {
      ...
      const storedLanguageCode = await AsyncStorage.getItem('tifto-language')
  ...
  async function onSelectedLanguage() {
    try {
      ...
      await AsyncStorage.setItem('tifto-language', languageCode)
      await AsyncStorage.setItem('tifto-language-name', languageVal)
